...
// L4 leaf level: one row per product (grouped across trips), expandable to the individual
// occurrences with a link back to each source invoice. No further drill.
export function SpendItems({ nodes, currency }: SpendItemsProps) {
  if (nodes.length === 0) {
    return (
  ...
  return (
    <ul className="spend-items" data-testid="spend-items">
      {nodes.map((node) => (
        <SpendItemRow key={node.id} node={node} currency={currency} />
      ))}
    </ul>
  )
}

function SpendItemRow({ node, currency }: { node: SpendNode; currency: string | null }) {
