...
import type { IAppUserRepository } from '@core/ports/IAppUserRepository';
...
describe('UserProvisioningService', () => {
  ...
  beforeEach(() => {
    mockUserRepo = { findByCognitoSub: vi.fn(), insertUser: vi.fn() };
  ...
  it('inserts user with ACTIVE status when slot is claimed', async () => {
    ...
    expect(mockUserRepo.insertUser).toHaveBeenCalledWith('sub-1', 'user@example.com', 'ACTIVE');
  ...
  it('inserts user with WAITLIST status when cap is reached', async () => {
    ...
    expect(mockCounter.tryClaimSlot).toHaveBeenCalledWith(100);
    expect(mockUserRepo.insertUser).toHaveBeenCalledWith('sub-2', 'waitlisted@example.com', 'WAITLIST');
    expect(result.status).toBe('WAITLIST');
  });

  it('passes correct cognitoSub and email to insertUser', async () => {
    mockCap.getMaxFreeUsersCap.mockResolvedValue(50);
    mockCounter.tryClaimSlot.mockResolvedValue(true);
    mockUserRepo.insertUser.mockResolvedValue('user-uuid-3');

    await sut.provisionUser('google|12345', 'another@test.nl');
