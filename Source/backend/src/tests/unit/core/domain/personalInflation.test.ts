...
describe('computePersonalInflation', () => {
  it('returns the median of per-product price ratios as a signed percentage', () => {
    ...
    const result = computePersonalInflation(basket);
    expect(result.personalInflationPct).toBe(10);
  ...
  it('averages the two middle ratios for an even-sized basket', () => {
    ...
    expect(computePersonalInflation(basket).personalInflationPct).toBe(15);
  });
  ...
  it('rounds to one decimal place', () => {
    ...
    expect(computePersonalInflation(basket).personalInflationPct).toBe(7.7);
  });
});
