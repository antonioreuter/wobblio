...
import { InvoiceNotDeletableError, InvoiceNotFoundError } from '../../domain/errors';
...
export class DeleteInvoiceService {
  ...
  async delete(invoiceId: string): Promise<void> {
    ...
    if (!isDeletable(invoice.status)) throw new InvoiceNotDeletableError(invoiceId, invoice.status);
