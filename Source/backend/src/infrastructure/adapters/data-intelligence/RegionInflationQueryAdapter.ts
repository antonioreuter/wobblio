...
// design (§6.5). Prices are the pack price paid (fix 09/01) — the sole signal, never per-unit.
// Quarantined (unpromoted catalog) and discounted rows are excluded so the index tracks promoted,
// regular shelf price. Only products meeting the k-observation quorum in BOTH periods survive. Not
// integration-tested yet — verify against seeded observations before relying on the number.
      ...
      `WITH obs AS (
       ...
       agg AS (
         ...
         SELECT o.product_id, o.bucket,
                percentile_cont(0.5) WITHIN GROUP (ORDER BY o.pack_price) AS med,
                COUNT(*) AS n
         FROM obs o
